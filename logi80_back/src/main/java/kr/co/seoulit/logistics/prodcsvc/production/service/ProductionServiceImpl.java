...
public class ProductionServiceImpl implements ProductionService {
	...
	private final MpsResMapstructor mpsResMapstructor;
    // private final MpsReqMapstructor mpsReqMapstructor;
	 ...
	 */
	@Override
//	@Cacheable(key = "#mpsReqTO",value = "mpsList")
	public ArrayList<MpsResTO> getMpsMainList(MpsReqTO mpsReqTO) {
		// NOTE: mrp적용상태가 null인놈 찾기
		System.out.println("로그 찍어봄");
		if(mpsReqTO.getIncludeMrpApply().equals("excludeMrpApply"))
			return mpsResMapstructor.toDto(mpsRepository.findByMpsPlanDateBetweenAndMrpApplyStatusIsNull(mpsReqTO.getStartDate(),mpsReqTO.getEndDate()));

		// NOTE: 날짜만으로 검색
		return mpsResMapstructor.toDto(mpsRepository.findByMpsPlanDateBetween(mpsReqTO.getStartDate(),mpsReqTO.getEndDate()));
	}

	/**
	 * NOTE: MRP에서 MPS 조회 후 업데이트를 하기때문에 CachePut 사용해야해서 분리함
	 *  Mapstruct적용 캐시 추가*/
	@Override
	@CachePut(key = "#mpsReqTO",value = "mpsList")
	...
	@Override
	public List<MpsEntity> convertContractDetailToMps(
			ArrayList<ContractDetailInMpsAvailableTO> contractDetailInMpsAvailableList) {

		List<MpsEntity> insertList = new ArrayList<>();

		/* NOTE: getNewMpsNo()메서드를 쓰지 않는 이유는 이렇게하면 커리문이 한번만 나가는데 이전에는 for문안으로 들어가여 여러번 동일한
		    커리문을 호출하였음!*/
		ContractDetailInMpsAvailableTO contractDetail = contractDetailInMpsAvailableList.get(0);
		MpsEntity mps = mpsMapper.getNewMpsNo(contractDetail.getMpsPlanDate());
		int no = 1;
		if (mps != null) {
		...
		for (ContractDetailInMpsAvailableTO bean : contractDetailInMpsAvailableList) {
			...
			MpsEntity newMpsBean = new MpsEntity();
			// 수주상세번호 생성 mps
			...
			if (bean.getContractDetailNo() != null) {
				...
				// jpa사용
				Optional<ContractDetailEntity> contractDetailNo = contractDetailRepository
	...
	public HashMap<String, Object> convertSalesPlanToMps(
		...
		ArrayList<MpsEntity> mpsTOList = new ArrayList<>();

		MpsEntity newMpsBean = null;

		for (SalesPlanInMpsAvailableTO bean : salesPlanInMpsAvailableList) {

			newMpsBean = new MpsEntity();
	...
	public HashMap<String, Object> registerMrp(String mrpRegisterDate, String mpsNoList) {
		...
		for (String mpsNo : SplitMpsNo) {
			Optional<MpsEntity> mpsTO = mpsRepository.findByMpsNo(mpsNo);
